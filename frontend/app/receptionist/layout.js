...
export default function ReceptionistLayout({ children }) {
  useEffect(() => {
    ...
    if (!u || ![1, 5].some(r => roles.includes(r))) window.location.href = '/login';
  }, []);
    ...
    <div style={{ display: 'flex', minHeight: '100vh', background: '#f5f8fc', fontFamily: 'Inter, sans-serif' }}>
      <aside style={{ width: 220, background: '#0f1f3d', color: '#fff', padding: '1.5rem 0', display: 'flex', flexDirection: 'column' }}>
        ...
        <nav style={{ flex: 1, padding: '1rem 0' }}>
          {NAV.map(n => (
            <a key={n.href} href={n.href} style={{ display: 'block', padding: '.6rem 1.5rem', color: '#cbd5e1', fontSize: '.875rem', textDecoration: 'none', borderLeft: '3px solid transparent' }}
